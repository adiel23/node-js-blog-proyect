...
export class Like {
    ...
    static async findByUserIdAndCommentId(userId, commentId) {
        try {
            ...
            const [results] = await connection.query('select * from comment_likes where userId = ? and commentId = ?', [userId, commentId]);
    ...
    async delete() {
        try {
            return await connection.query('delete from comment_likes where id = ?', [this.id]);
        } catch (err) {
    ...
    async insert() {
        try {
            ...
            const [results] = await connection.query('insert into comment_likes (commentId, userId) values (?, ?)', [this.commentId, this.userId]);
